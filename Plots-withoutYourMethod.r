...
PathToClonePhyTester='C:\\Users\\tuf78332\\Documents\\GitHub\\ClonePhyTester\\' ###please change
jpeg('plot.jpg') ##please change

#par(mfrow=c(3,1))
MetLs=c("CloneFinder","MACHINA","TreeOmics",  "LICHeE", "MixPhy", "PhyloWGS",  "Cloe")
#C:\\Users\\tuf78332\\Documents\\GitHub\\ClonePhyTester\\
Seq0 = read.xls(paste(PathToClonePhyTester,"Summary.xlsx",sep=''), sheet = 'Sequential_MutPair_count', header = TRUE)
...
Par0 = read.xls(paste(PathToClonePhyTester,"Summary.xlsx",sep=''), sheet = 'parallel_MutPair_count', header = TRUE)
...
Con0 = read.xls(paste(PathToClonePhyTester,"Summary.xlsx",sep=''), sheet = 'concurrent_MutPair_count', header = TRUE)
...
ML0 = read.xls(paste(PathToClonePhyTester,"Summary.xlsx",sep=''), sheet = 'MLTED', header = TRUE)
...
TV0 = read.xls(paste(PathToClonePhyTester,"Summary.xlsx",sep=''), sheet = 'TreeVec', header = TRUE)
...
RF0 = read.xls(paste(PathToClonePhyTester,"Summary.xlsx",sep=''), sheet = 'RF', header = TRUE)
...
ggplot(Ta, aes(x=Method, y=ErrorRate, color=Method)) +geom_boxplot()+facet_grid(ErrorScore~Sim, scales = "free_y")+ scale_x_discrete(limits=MetLs)+scale_color_manual(values=c("#6b9b87", "#5871ba", "#91c452", "#7f57b0", "#d0ba4a", "#e4616e", "#d29a44"))+ theme(axis.text.x = element_text(angle = 45, hjust = 1),legend.position = "none")
