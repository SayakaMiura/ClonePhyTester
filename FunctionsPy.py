...
def MakeTopologeTree(Nwk,Rscript):
...
def Meg2MP(Meg,RootTaxa,Rpath):
    ...
    if os.path.exists(MegID+'.nwk')==True:
        MakeTopologeTree(MegID+'.nwk',Rpath)
        os.remove(MegID+'.nwk')
...
def RootTree(OriNwk, RootTaxa):	
        ...
        Tree_res=open('test1.nwk','r').readlines()[0]
        os.remove('test.nwk')
        os.remove('test1.nwk')
        return Tree_res

def Build_true_and_inferred_phylogeny(TrueFound_Ls,PairList,TrueMeg,InfMeg,Rpath):
   ...
   UpMeg(TruClo2Seq,TrueFound_Ls,'True.meg')
   UpMeg(CorrectSeq_rename,TrueFound_Ls,'Inf.meg')
   TrueMultiTree=Meg2MP('True.meg','#Normal',Rpath)
   InfMultiTree=Meg2MP('Inf.meg','#Normal',Rpath)
   os.remove('True.meg')
   os.remove('Inf.meg')
...
def MakeMLTEin(Meg,OriTree,OutMLTEDfile,Rpath):
  if OriTree=='NA':
...
def Compute_RF(TrueTree,InfTree):
    if TrueTree!='NA':
...
def Compute_MLTED(Ttree,InfMegTMP,TruMegTMP,Rpath,MLTEDpath):
    if Ttree=='NA': return 'NA'
    else:
     Itree=MakeMLTEin(InfMegTMP,'NA',InfMegTMP[:-4]+'_MLTEin.txt',Rpath)
     Ttree=Ttree.replace('Normal:','hg19:')
     Ttree_redun=MakeMLTEin(TruMegTMP,Ttree,TruMegTMP[:-4]+'_MLTEin.txt',Rpath)
     MLTED='NA'
...
def Compute_TreeVec(Ttree,InfMegTMP,Inf2TruClo,TrueCloneLs,Tru2Inf2GE,Rpath):
    if Ttree=='NA': return 'NA'
    else:
      Ttree=Ttree.replace('Normal:','hg19:')
      InfTree, InfCloneLs=DupRenameClone_MPtree(InfMegTMP,Inf2TruClo,TrueCloneLs,Tru2Inf2GE,Rpath)
      if InfTree=='NA': return 'NA'
      else:
        TreeVec=Dotreespace(Ttree,InfTree,TrueCloneLs,InfCloneLs,Rpath)
...
def Dotreespace(TTre,ITre,TCloLs,ICloLs,Rpath):
    ...
    if os.path.exists('test.out')==True:
